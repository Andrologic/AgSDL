"""Modular candidate-1 closed shapes. Semantic rules live in reader.py."""
import re
...
SCHEMAS.update({
    ...
    'Invoke': record(id='text', kind=enum('invoke'), agent='Ref', interface='Ref', operation='text', action='Ref', resources=array('Ref', 1), principal='Ref', context='Binding', inputs='Ports', outputs='Ports', bindings=('map', 'Binding'), success='text', failure='text'),
    'Condition': record(id='text', kind=enum('condition'), test='Binding', true='text', false='text', failure='text'),
    'Approval': record(id='text', kind=enum('approval'), requirement='Ref', call='text', timeoutMs='positive', approved='text', denied='text', failure='text'),
    ...
    'Interface': record(operations=array('Operation', 1)),
    'Operation': record(id='text', direction=enum('inbound', 'outbound', 'bidirectional'), mode=enum('request-response'), action='Ref', inputs='Ports', outputs='Ports'),
    'ApprovalRequirement': record(approvers=array('Ref', 1), validForMs='positive'),
    'Runtime': record(configurations=array('Configuration'), **{'selected?': 'text'}),
    'Configuration': record(id='text', graph='Key', agents=array('AgentBinding')),
    'AgentBinding': record(agent='Ref', engine=union('Edition', 'null'), parameters='any', requires=array('Edition'), claims=array('CapabilityClaim'), tools=array('ToolBinding'), applications=array('Application')),
    'ToolBinding': record(tool='Ref', choices=array('Implementation'), **{'selected?': 'text'}),
    'Implementation': record(id='text', implementation='Edition', parameters='any', claims=array('CapabilityClaim')),
    'Application': record(content='Ref', adapter='Edition', parameters='any'),
    'CapabilityClaim': record(capability='Edition', status=enum('supported', 'unsupported', 'unknown'), evidence=union('hash', 'null')),
    'ToolPayload': record(action='Ref', inputs='Ports', outputs='Ports', effects=enum('none', 'external', 'unknown'), failures=array('text'), requires=array('Edition')),
    'InstructionsPayload': record(target=enum('Agent'), at=enum('before-invoke'), format='Edition', body='text', requires=array('Edition')),
    'SkillPayload': record(inputs='Ports', outputs='Ports', preconditions='text', completion='text', dependencies=array('Ref'), tools=array('Ref'), requires=array('Edition')),
})
