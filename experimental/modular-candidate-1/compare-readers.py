#!/usr/bin/env python3
"""Compare modular candidate reports and corpus oracles, without deriving semantics."""
...
LEGACY_PATH = ROOT.parent / "candidate-2" / "compare-readers.py"
SPEC = importlib.util.spec_from_file_location("agsdl_candidate2_neutral", LEGACY_PATH)
...
def main(argv=None):
    ...
    summary = {"cases": completed, "readers": [item[0] for item in readers], "blocked": blocked, "failures": failures,
               "scope": "proposal-0013 report contract, modular corpus oracles, exact slices and exchange bytes; no semantic derivation or runtime execution"}
