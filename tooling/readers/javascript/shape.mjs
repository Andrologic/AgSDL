...
import * as Base from './document-shape.mjs';
...
export const Document = Base.DocumentFor('agsdl-0.1.0');
...
export const CapabilityClaim = record({
  capability: Edition,
  status: literal('supported', 'unsupported', 'unknown'),
  evidence: nullable(hash),
});
export const Implementation = record({
  id: text,
  implementation: Edition,
  parameters: any,
  claims: array(CapabilityClaim),
});
export const ToolBinding = record({
  tool: Ref,
  choices: array(Implementation),
...
export const AgentBinding = record({
  agent: Ref,
  engine: nullable(Edition),
  parameters: any,
  requires: array(Edition),
  claims: array(CapabilityClaim),
...
export const Tool = record({
  action: Ref,
  inputs: Ports,
  outputs: Ports,
  effects: literal('none', 'external', 'unknown'),
  failures: array(text),
  requires: array(Edition),
});
export const Instructions = record({
  target: literal('Agent'),
  at: literal('before-invoke'),
  format: Edition,
  body: text,
  requires: array(Edition),
});
...
export function errors(type, v, p = '') {
  ...
  if (type.union) {
    const all = type.union.map((t) => errors(t, v, p));
    return all.some((e) => !e.length) ? [] : all.at(-1);
  }
  if (type.array) {
  ...
  for (const [k, t] of Object.entries(type.fields))
    if (!Object.hasOwn(v, k)) {
      if (!t.optional) out.push(p);
    } else out.push(...errors(t.optional || t, v[k], pointer(p, k)));
  for (const k of Object.keys(v))
    if (!Object.hasOwn(type.fields, k)) out.push(pointer(p, k));
  return [...new Set(out)];
}
