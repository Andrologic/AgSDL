...
export function validateR(ctx, inventory) {
  ...
  const state = (pointer, value, detail = value) =>
    inventory.states.push({ input: ctx.id, pointer, state: value, detail });

  if (ctx.error || !S.object(ctx.tree)) {
    for (const rule of RULES.filter((rule) => rule !== 'X-MODE'))
      result.mark(rule, 'blocked', '');
    return { result: result.finish(), interpreted };
  }
  if (!S.has(ctx.tree, 'runtime')) {
    for (const rule of RULES.filter((rule) => rule !== 'X-MODE'))
      result.mark(rule, 'excluded', '/runtime');
    return { result: result.finish(), interpreted };
  }
  ...
  if (!S.object(runtime)) {
    for (const rule of RULES.filter(
      (rule) => !['P-SHAPE', 'X-MODE'].includes(rule),
    ))
      result.mark(rule, 'blocked', '/runtime');
    return { result: result.finish(), interpreted };
  }
  ...
  const graphRows = Array.isArray(ctx.tree.graphs)
    ? ctx.tree.graphs.map((value, index) => ({
        value,
        pointer: `/graphs/${index}`,
      }))
    : null;
  const graphIndexReadable =
    graphRows !== null &&
    graphRows.every(
      (row) => S.object(row.value) && S.valid(S.Key, row.value.definition),
    );

  function graphFor(configuration, pointer) {
    if (
      !S.valid(S.Key, configuration?.graph) ||
      !graphRows ||
      !graphIndexReadable
    ) {
      result.mark('R-SELECTION', 'blocked', pointer);
    ...
    const matches = graphRows.filter((row) =>
      equal(row.value.definition, configuration.graph),
    );
    if (!matches.length) {
  ...
  function resolveRef(value, kind, rule, pointer, statePointer = pointer) {
    ...
    if (!declaration(ctx, value, kind, result, rule, pointer)) {
      ...
      if (!localIndexComplete(ctx) || found?.length > 1)
        return { status: 'blocked', ref: value };
      if (found?.length === 1 && kind && !S.valid(S.Kind, found[0].v.kind))
        return { status: 'blocked', ref: value };
    ...
    return found
      ? { status: 'local', ref: value, ...found }
      : { status: 'blocked', ref: value };
  }
  ...
  function claimMap(claims, rule, pointer, reportDuplicates = true) {
    ...
    for (const claim of claims) {
      ...
      if (!S.valid(S.Edition, claim?.capability)) {
        indexComplete = false;
        continue;
      }
      result.mark(rule);
      ...
      if (seen.has(identity)) {
        if (reportDuplicates)
          result.find(rule, pointer, 'Duplicate capability claim');
        map.delete(identity);
  ...
  function projection(graph, configurationPointer) {
    ...
    for (const step of graph.value.steps) {
      if (
        !S.object(step) ||
        !['invoke', 'condition', 'approval', 'end'].includes(step.kind) ||
        (step.kind === 'invoke' && !S.valid(S.Ref, step.agent))
      ) {
        result.mark('R-BINDING', 'blocked', configurationPointer);
        return null;
      }
      if (
        step.kind === 'invoke' &&
        !agents.some((agent) => canonical(agent) === canonical(step.agent))
      )
  ...
  for (const [ci, configuration] of configurations.entries()) {
    ...
    if (!S.object(configuration)) {
      for (const rule of ['R-SELECTION', 'R-BINDING', 'R-TOOL', 'R-CONTENT'])
        result.mark(rule, 'blocked', cp);
      analyses.set(configuration, {
        cp,
        groups: new Map(),
        used: null,
    ...
    if (!Array.isArray(configuration.agents))
      for (const rule of ['R-BINDING', 'R-TOOL', 'R-CONTENT'])
        result.mark(rule, 'blocked', cp);
    ...
    for (const [ai, binding] of bindings.entries()) {
      ...
      if (!S.object(binding)) {
        for (const rule of ['R-BINDING', 'R-TOOL', 'R-CONTENT'])
          result.mark(rule, 'blocked', ap);
        structuralBlocked = true;
        bindingIndexComplete = false;
        continue;
      }
      const agent = resolveRef(
        binding.agent,
        'Agent',
        'R-BINDING',
        ap,
      ...
      if (S.valid(S.Ref, binding.agent)) {
        ...
        if (matches.length)
          result.find('R-BINDING', ap, 'Duplicate AgentBinding');
        matches.push(item);
    ...
    if (used && bindingIndexComplete) {
      ...
      if (
        needed.size !== groups.size ||
        [...needed].some((identity) => !groups.has(identity))
      ) {
        result.find(
          'R-BINDING',
          cp,
          'AgentBinding coverage differs from graph Agents',
        );
      }
    }
    analyses.set(configuration, {
  ...
  function analyzeBinding(item, assess, forceAssessmentBlocked) {
    ...
    if (agent.status === 'local') {
      if (!Array.isArray(ctx.tree.relations)) {
        ...
        for (const relation of relationRows) {
          ...
          if (
            S.valid(S.Key, relation.v?.source) &&
            !equal(relation.v.source, agent.v.key)
          )
            continue;
          if (relation.v?.relation === 'uses') toolRelationsBlocked = true;
          else if (
            relation.v?.relation === 'directedBy' ||
            !['actsAs', 'exposes', 'contains'].includes(relation.v?.relation)
          ) {
    ...
    function rememberTool(ref, request, rule, statePointer) {
      ...
      if (!requiredTools.has(identity))
        requiredTools.set(identity, { ref, found, request });
    ...
    function visitContent(ref, request, statePointer = request) {
      ...
      const identity = canonical(ref);
      if (complete.has(identity) || visiting.has(identity))
        return content.get(identity)?.found || found;
      ...
      if (!info?.value) {
        ...
        const ownRequirements = editionMap(
          info.value.requires,
          'R-CONTENT',
          info.pointer,
        );
        for (const [capability, value] of ownRequirements.map)
          engineRequirements.set(capability, value);
        if (ownRequirements.blocked) assessmentBlocked = true;
        if (found.v.kind === 'Instructions') {
          if (S.valid(S.Edition, info.value.format))
            engineRequirements.set(
              edition(info.value.format),
              info.value.format,
            );
          ...
          if (!Array.isArray(info.value.dependencies)) {
            ...
            for (const [
              index,
              dependency,
            ] of info.value.dependencies.entries()) {
              ...
              visitContent(
                dependency,
                info.pointer,
                `${info.pointer}/dependencies/${index}`,
              );
            }
          }
    ...
    if (agent.status === 'local') {
      for (const relation of relationRows.filter(
        (row) => usableRelation(row) && equal(row.v.source, agent.v.key),
      )) {
        if (
          relation.v.relation === 'directedBy' &&
          ['Instructions', 'Skill'].includes(relation.v.expectedKind)
        )
          rememberContent(
            relation.v.target,
            relation.p,
        ...
        if (
          relation.v.relation === 'uses' &&
          relation.v.expectedKind === 'Tool'
        )
          rememberTool(
            relation.v.target,
            relation.p,
    ...
    for (const [identity, entry] of content) {
      ...
      if (cycle && entry.info)
        result.find('R-CONTENT', entry.info.pointer, 'Skill dependency cycle');
    }

    const applications = Array.isArray(binding.applications)
    ...
    function observeAdditionalContent(found, request) {
      if (
        found.status !== 'local' ||
        !['Instructions', 'Skill'].includes(found.v.kind)
      )
        return;
      ...
      for (const field of ['dependencies', 'tools']) {
        ...
        for (const [index, ref] of refs.entries()) {
          ...
          const target = resolveRef(
            ref,
            field === 'tools' ? 'Tool' : null,
            'R-CONTENT',
            info.pointer,
            `${info.pointer}/${field}/${index}`,
          );
          if (field === 'tools')
            extraTools.set(targetIdentity, {
              found: target,
              request: info.pointer,
            });
          else {
            extraEdges.push([identity, targetIdentity]);
            if (target.status === 'local' && !S.valid(S.Kind, target.v.kind))
              result.mark('R-CONTENT', 'blocked', info.pointer);
            else if (
              target.status === 'local' &&
              !['Instructions', 'Skill'].includes(target.v.kind)
            )
              result.find(
                'R-CONTENT',
                info.pointer,
                'Content target has wrong kind',
              );
    ...
    for (const [index, application] of applications.entries()) {
      ...
      if (S.valid(S.Edition, application.adapter))
        engineRequirements.set(
          edition(application.adapter),
          application.adapter,
        );
      else assessmentBlocked = true;
      ...
      applicationPositions.set(identity, [
        ...(applicationPositions.get(identity) || []),
        index,
      ]);
    }
    ...
    for (const [identity, entry] of extraContent) {
      ...
      while (pending.length) {
        ...
        seen.add(next);
        pending.push(
          ...extraEdges.filter(([from]) => from === next).map(([, to]) => to),
        );
      }
    }
    ...
    if (applicationIndexComplete) {
      ...
      for (const [, dependency] of extraEdges)
        if (!applicationPositions.has(dependency))
          result.find('R-CONTENT', ap, 'Required Application missing');
    }
    ...
    for (const [identity, entry] of observedContent) {
      if (
        entry.kind !== 'Skill' ||
        !Array.isArray(entry.info?.value?.dependencies)
      )
        continue;
      ...
      for (const dependency of entry.info.value.dependencies) {
        ...
        const prerequisites =
          applicationPositions.get(canonical(dependency)) || [];
        for (const dependent of dependents)
          if (!prerequisites.some((prerequisite) => prerequisite < dependent)) {
            if (
              unreadableApplicationPositions.some(
              ...
              result.mark(
                'R-CONTENT',
                'blocked',
                `${ap}/applications/${dependent}`,
              );
            else if (prerequisites.length)
              result.find(
                'R-CONTENT',
                `${ap}/applications/${dependent}`,
                'Skill dependency must appear earlier',
              );
          }
      }
    }
    if (contentClosureComplete) {
      for (const [identity, positions] of applicationPositions)
        if (!requiredContent.has(identity)) {
          for (const position of positions)
            result.find(
              'R-CONTENT',
              `${ap}/applications/${position}`,
              'Application is not reachable from Agent direction',
            );
        }
    } else {
      for (const [identity, positions] of applicationPositions)
        if (!requiredContent.has(identity)) {
          for (const position of positions)
            result.mark(
              'R-CONTENT',
              contentClosureBlocked ? 'blocked' : 'excluded',
              `${ap}/applications/${position}`,
            );
        }
    }
    for (const [identity, entry] of observedContent)
      if (
        entry.kind === 'Skill' &&
        (!Array.isArray(entry.info?.value?.dependencies) ||
          entry.info.value.dependencies.some(
            (dependency) => !S.valid(S.Ref, dependency),
          ))
      ) {
        for (const position of applicationPositions.get(identity) || [])
          result.mark('R-CONTENT', 'blocked', `${ap}/applications/${position}`);
      }
    ...
    for (const [index, toolBinding] of tools.entries()) {
      ...
      for (const [choiceIndex, choice] of choices.entries()) {
        ...
        let selectedChoiceBlocked =
          choiceClaims.blocked || !S.valid(S.Implementation, choice);
    ...
    const missingTool =
      toolIndexComplete &&
      [...requiredTools.keys()].some((identity) => !toolGroups.has(identity));
    ...
    if (missingTool || (toolClosureComplete && extraTool))
      result.find(
        'R-TOOL',
        ap,
        'ToolBinding coverage differs from required Tools',
      );

    const toolPayloads = new Map();
    const observedTools = new Map([...extraTools, ...requiredTools]);
    for (const [identity, matches] of toolGroups)
      if (!observedTools.has(identity)) {
        const binding = matches[0];
        observedTools.set(identity, {
          found: binding.found,
          request: binding.tp,
        });
      }
    for (const [identity, requirement] of observedTools) {
      ...
      if (info?.value) {
        const requirementsForTool = editionMap(
          info.value.requires,
          'R-TOOL',
          info.pointer,
        );
        ...
        if (Array.isArray(info.value.failures)) {
          const readableFailures = info.value.failures.filter(
            (failure) => typeof failure === 'string' && failure.length,
          );
          if (readableFailures.length !== info.value.failures.length) {
            result.mark('R-TOOL', 'blocked', info.pointer);
            blocked = true;
          }
          if (new Set(readableFailures).size !== readableFailures.length)
            result.find('R-TOOL', info.pointer, 'Duplicate Tool failure');
        } else {
          result.mark('R-TOOL', 'blocked', info.pointer);
          blocked = true;
        }
        const action = resolveRef(
          info.value.action,
          'Action',
          'R-TOOL',
          info.pointer,
        ...
        toolPayloads.set(identity, {
          info,
          requirements: requirementsForTool.map,
          blocked,
        });
      } else
        toolPayloads.set(identity, {
          ...
          blocked: requirement.found.status !== 'external',
        });
    }

    if (!assess) return;
    for (const toolBinding of unindexedTools)
      assessTool(toolBinding, null, true, false);
    for (const [identity, matches] of toolGroups) {
      const required = requiredTools.has(identity);
      const payloadInfo = toolPayloads.get(identity);
      const uncertainMembership =
        !required &&
        !toolClosureComplete &&
        !toolClosureBlocked &&
        toolClosureUnknown;
      for (const toolBinding of matches)
        assessTool(
          toolBinding,
          payloadInfo,
          forceAssessmentBlocked ||
            matches.length !== 1 ||
            (!required && (toolClosureComplete || toolClosureBlocked)),
          uncertainMembership,
        );
    }
    assessAgent();
    ...
    function recordAssessment(pointer, values, blocked = false) {
      ...
      if (values.includes('incompatible'))
        result.find(
          'R-COMPATIBILITY',
          pointer,
          'Declared capability incompatible',
        );
      if (values.some((value) => ['not-provided', 'unknown'].includes(value)))
        result.find(
          'R-COMPATIBILITY',
          pointer,
    ...
    function assessTool(
      ...
      if (selection.status === 'absent') {
        const values = uncertainMembership
      ...
      const values = evaluate(
        payloadInfo?.requirements || new Map(),
        selection.claims,
        true,
        uncertainMembership ||
          toolBinding.found.status === 'external' ||
          payloadInfo?.info?.value?.effects === 'unknown',
      );
      recordAssessment(
        tp,
        values,
        forcedBlocked ||
          toolBinding.choiceBlocked ||
          selection.blocked ||
    ...
    for (const [identity, matches] of analysis.groups) {
      for (const item of matches)
        analyzeBinding(
          item,
          isSelected,
          analysis.structuralBlocked ||
            matches.length !== 1 ||
            !needed.has(identity),
        );
    }
    for (const item of analysis.unindexed || [])
      analyzeBinding(item, isSelected, true);
    if (isSelected && analysis.structuralBlocked)
      result.mark('R-COMPATIBILITY', 'blocked', analysis.cp);
  }
